...
test("handoff validates full binding and fences every former-owner mutation", () => {
  ...
  store.finishOperation("from",lease.leaseId,2,"pinned-effect");
  const packet={resource:lease.resource,baseRevision:lease.baseRevision,scope:lease.scope,candidateRevision:"candidate",liveOperation:lease.operation,liveHandle:"",checkpoint:"checkpoint:42",grantDependency:grant,grantVersion:1,recipientGrant,recipientGrantVersion:1,forbiddenOverlap:["/repo"],tests:["test-ref"],evidence:["evidence:42"],remainingGap:"",nextGate:"continue",expiresAt:lease.expiresAt};
  try {
    for(const delta of [{resource:"wrong"},{baseRevision:"wrong"},{scope:["/other"]},{liveOperation:"wrong"},{liveHandle:"wrong"},{grantVersion:2},{recipientGrantVersion:2},{checkpoint:""},{evidence:[]}]) {
      assert.throws(()=>store.handoff("from",lease.leaseId,3,"to",{...packet,...delta}));assert.equal(store.get(lease.leaseId)?.ownerThread,"from");
    }
    allowRecipient=false;assert.throws(()=>store.handoff("from",lease.leaseId,3,"to",packet));allowRecipient=true;
    now+=60000;assert.throws(()=>store.handoff("from",lease.leaseId,3,"to",packet));now-=60000;
    ...
    assert.throws(()=>store.handoff("from",lease.leaseId,3,"to",packet),/receipt denied/);assert.equal(store.get(lease.leaseId)?.version,3);
    db.exec("drop trigger deny_handoff");
    const receipt=store.handoff("from",lease.leaseId,3,"to",packet);
    ...
    const current=store.get(lease.leaseId)!;assert.equal(current.ownerThread,"to");assert.equal(current.operationHandle,undefined);assert.deepEqual(current.grant,recipientGrant);
    assert.throws(()=>store.handoff("from",lease.leaseId,3,"to",packet));
    assert.throws(()=>store.finishOperation("from",lease.leaseId,4,"pinned-effect"));
    assert.throws(()=>store.renew("from",lease.leaseId,4,new Date(now+120000).toISOString()));
    assert.throws(()=>store.release("from",lease.leaseId,4));
    assert.throws(()=>store.reconcile("from",lease.leaseId,4,{leaseId:lease.leaseId,ownerThread:"from",operationHandle:"pinned-effect",operation:"write",baseRevision:"base",leaseVersion:4,state:"finished"}));
    assert.equal(store.get(lease.leaseId)?.version,4);
    store.putGrantEvidence("from",{...grant,evidenceHash:"sender-revoked"},1);
    assert.deepEqual(new ControlPlaneHandoff(store).readback("to",lease.leaseId,3,4).receipt,receipt);
  } finally {db.close();}
});
...
test("handoff readback survives reopen and distinguishes history from current pin", async () => {
  ...
  try {
    ...
    now-=60001;
    assert.throws(()=>store.handoff("to",lease.leaseId,3,"third",{...packet,liveHandle:"new-operation"}), /reconcile/);
    store.finishOperation("to",lease.leaseId,3,"new-operation");
    const thirdReceipt=store.handoff("to",lease.leaseId,4,"third",packet);
    assert.throws(()=>facade.readback("to",lease.leaseId,1,5));
    assert.throws(()=>facade.readback("third",lease.leaseId,1,5));
    assert.deepEqual(facade.readback("third",lease.leaseId,4,5).receipt,thirdReceipt);
    store.putGrantEvidence("third",{...grant,evidenceHash:"replacement"},1);
    store.putGrantEvidence("third",grant,2);
    assert.throws(()=>facade.readback("third",lease.leaseId,4,5), /grant/);

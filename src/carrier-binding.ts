    ...
    try {
      ...
      const localOwnership=operations.createOwnershipStore({
        ...
        verifyReconciliationEvidence: (evidence,lease,owner)=>{
          ...
          if(!closed || closed.cutoverId!==input.cutoverId || closed.phase!=="closed" || !closed.expiredPreparedNoEffect) return false;
          const detail=JSON.stringify({kind:"cutover_expired_prepared_no_effect",cutoverId:input.cutoverId,requestHash:correlation.requestHash,terminalRecordHash:cutoverTerminalRecordHash(closed),drainObserved:false,restartRequested:false,restartScheduled:false});
          return owner.ownerThread===binding.row.id && lease.leaseId===correlation.leaseId && lease.ownerThread===binding.row.id && lease.operation==="cutover_start" && lease.baseRevision===binding.contract.baseRevision && lease.operationHandle===correlation.operationHandle && evidence.leaseId===lease.leaseId && evidence.ownerThread===lease.ownerThread && evidence.operationHandle===correlation.operationHandle && evidence.operation==="cutover_start" && evidence.baseRevision===lease.baseRevision && evidence.leaseVersion===correlation.pinnedLeaseVersion && evidence.state==="finished" && evidence.detail===detail;
        },
      });
      ...
      const recovered=cutoverStore.recoverExpiredPreparedNoEffect({cutoverId:input.cutoverId,recoveredBy:"host-local-owner"});
      const terminalHash=cutoverTerminalRecordHash(recovered.record);
      const evidence:ReconciliationEvidence={leaseId:correlation.leaseId,ownerThread:binding.row.id,operationHandle:correlation.operationHandle,operation:"cutover_start",baseRevision:binding.contract.baseRevision,leaseVersion:correlation.pinnedLeaseVersion,state:"finished",detail:JSON.stringify({kind:"cutover_expired_prepared_no_effect",cutoverId:input.cutoverId,requestHash:correlation.requestHash,terminalRecordHash:terminalHash,drainObserved:false,restartRequested:false,restartScheduled:false})};

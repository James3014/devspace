...
const options = { resolveResourceIdentity: (input: Parameters<ControlPlaneOwnershipStore["acquire"]>[1]) => input, resolveOwnerContext: (value: unknown) => { const thread = (value as { thread?: unknown })?.thread; return typeof thread === "string" ? { ownerThread: thread } : undefined; }, verifyGrantEvidence: (value: GrantEvidenceReference) => value.repository === "owner/repo" && value.goal === "goal" && value.coordinatorThread === "coord" && value.evidenceHash === "hash" };
...
test("C2 production filesystem identity rejects aliases and allows disjoint scopes", () => {
  const root=mkdtempSync(join(tmpdir(),"devspace-physical-"));const a=join(root,"a");const b=join(root,"b");const alias=join(root,"alias");mkdirSync(a);mkdirSync(b);symlinkSync(a,alias,"junction");
  const sqlite=db();const store=new ControlPlaneOwnershipStore(sqlite,{resolveOwnerContext:options.resolveOwnerContext,verifyGrantEvidence:options.verifyGrantEvidence});
  ...
  try {
    ...
    assert.throws(()=>store.acquire(context("three"),{...request,scope:[join(root,"missing")],idempotencyKey:"missing"}));
    rmSync(a,{recursive:true});assert.throws(()=>store.assertHeld(context("one"),lease.leaseId,1,lease.operation,lease.baseRevision));
  } finally {sqlite.close();rmSync(root,{recursive:true,force:true});}
});

test("C2 linked worktrees share branch identity despite different labels and scopes", () => {
  const root=mkdtempSync(join(tmpdir(),"devspace-branch-"));const repo=join(root,"repo");const linked=join(root,"linked");mkdirSync(repo);
  const git=(...args:string[])=>execFileSync("git",["-C",repo,...args],{stdio:"pipe"});
  git("init");git("-c","user.name=Fixture","-c","user.email=fixture@example.test","commit","--allow-empty","-m","initial");git("worktree","add","--detach",linked);
  const sqlite=db();const store=new ControlPlaneOwnershipStore(sqlite,{resolveOwnerContext:options.resolveOwnerContext,verifyGrantEvidence:options.verifyGrantEvidence});
  try {
    const first={...input([repo]),resourceKind:"branch",resourceId:"shared",resource:repo};store.acquire(context("one"),first);
    assert.throws(()=>store.acquire(context("two"),{...first,resource:linked,resourceId:"refs/heads/shared",scope:[linked],idempotencyKey:"alias"}));
    assert.throws(()=>store.acquire(context("two"),{...first,resourceId:"bad..ref",idempotencyKey:"bad"}));
  } finally {sqlite.close();rmSync(root,{recursive:true,force:true});}
});

test("C2 malformed persisted lease identity fails closed without resetting records", () => {

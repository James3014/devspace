...
export function createCoreMutationGuard(
  ...
  return {
    ...
    snapshot: async (input) => {
      ...
      if (!session) {
        throw new Error("[CORE_MUTATION_SESSION_REQUIRED] No active Core mutation session exists for this workspace.");
      }
      try {
    ...
    recordCandidate: async (input) => {
      ...
      const session = store.getActive(input.workspaceId);
      if (!session) {
        throw new Error("[CORE_MUTATION_SESSION_REQUIRED] Candidate provenance requires an active Core mutation session.");
      }
      try {
...
export function registerCoreMutationSessionTools(
  ...
  registerAppTool(
    ...
    "core_mutation_session_close",
    {
      ...
      description:
        "Close one exact Core mutation session. COMPLETE requires a clean workspace after bounded Candidate formation; ABANDON preserves provenance without creating a trusted completion claim.",
      inputSchema: {
        workspaceId: z.string(),
        sessionId: z.string(),
    ...
    async ({ workspaceId, sessionId, mode }, extra) => {
      ...
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `Core mutation session ${session.id} closed as ${session.status}.` }],
          structuredContent: publicSession(session),
        };
      } catch (error) {
        throw toolError(error);

...
const readyDelayMs = ${options.tuiReadyDelayMs ?? 0};
if (readyDelayMs > 0) {
...
let tuiReady = readyDelayMs === 0;
function emitReadyFrame() {
...
class ScriptedDeltaBackend implements GoalProcessBackend {
  ...
  private nextSnapshot(): ProcessSnapshot {
    if (this.index >= this.readySnapshots.length) return snapshotFor({});
...
class TerminalHandshakeBackend implements GoalProcessBackend {
  ...
  async start(input: StartCommandInput): Promise<ProcessSnapshot> {
    ...
    return snapshotFor({ output: "\u001b[6n\u001b]10;?\u001b\\\u001b]11;?\u001b\\\u001b[?u\u001b[c" });
  }
  async write(input: WriteStdinInput): Promise<ProcessSnapshot> {
    if (input.chars) {
      this.writes.push(input.chars);
      if (input.chars === EXPECTED_TERMINAL_HANDSHAKE_REPLY) {
        this.ready = true;
        return snapshotFor({ output: `model: gpt-5.6-sol medium\ndirectory: ${this.workspaceRoot}\nAsk Codex to do anything\n` });
      }
      if (this.ready && this.writes.join("").includes("/goal ")) return snapshotFor({ output: "Pursuing goal\n" });
    }
    return snapshotFor({});
  }
...
test("Codex Goal answers only the bounded startup terminal handshake before typing /goal", async () => {
  ...
  try {
    const started = await manager.start({ workspaceId: "ws_terminal_handshake", workspaceRoot: workspace, goal: "handshake first", model: "gpt-5.6-sol" });
    ...
    assert.ok(backend.startInputs[0]?.args?.includes("check_for_update_on_startup=false"));
...
test("Codex Goal accepts cursor-positioned boxed Codex 0.149.0 readiness with home-relative directory", async () => {
  ...
  const backend: GoalProcessBackend = {
    async start() { return snapshotFor({ output: ["\u001b[4;1H│ model:     gpt-5.6-sol minimal   /model to change │","\u001b[5;1H│ directory: ~ │","\u001b[9;1H› Ask Codex to do anything","\u001b[10;1H"].join("") }); },
    async write(input) { if (input.chars) { writes.push(input.chars); if (writes.join("").includes("/goal ")) return snapshotFor({ output: "Pursuing goal\n" }); } return snapshotFor({}); },
    terminate() {},
  };
  const manager = scriptedGoalManager(backend);
  try {
    const started = await manager.start({ workspaceId: "ws_boxed_real_tui", workspaceRoot: workspace, goal: "parse real tui", model: "gpt-5.6-sol" });
    assert.equal(started.goalActiveObserved, true);
...
test("Codex Goal lets a later cursor repaint supersede historical startup loading without a clear-screen", async () => {
  ...
  let pollCount = 0;
  const backend: GoalProcessBackend = {
    async start() { return snapshotFor({ output: "model: loading\ndirectory: loading\nAsk Codex to do anything\n" }); },
    async write(input) {
      if (input.chars) { writes.push(input.chars); if (writes.join("").includes("/goal ")) return snapshotFor({ output: "Pursuing goal\n" }); }
      else if (pollCount++ === 0) return snapshotFor({ output: "gpt-5.6-sol default · ~\n" });
      else if (pollCount === 2) return snapshotFor({ output: ["\u001b[4;1H│ model:     gpt-5.6-sol minimal   /model to change │","\u001b[5;1H│ directory: ~ │","\u001b[9;1H› Ask Codex to do anything","\u001b[10;1H"].join("") });
      return snapshotFor({});
    },
    terminate() {},
  };
  const manager = scriptedGoalManager(backend);
  try {
    const started = await manager.start({ workspaceId: "ws_loading_cursor_repaint", workspaceRoot: workspace, goal: "accept latest repaint", model: "gpt-5.6-sol" });
    assert.equal(started.goalActiveObserved, true);
...
test("Codex Goal keeps a coherent ready frame stable across decorative MCP spinner deltas", async () => {
  ...
  const backend = new ScriptedDeltaBackend([{ output: ready }, { output: "Starting MCP servers (1/3)\n" }, { output: "⠋ nexus-devspace-mcp\n" }], { readyText: ready });
  const manager = scriptedGoalManager(backend);
  try {
    const started = await manager.start({ workspaceId: "ws_decorative_spinner_stability", workspaceRoot: workspace, goal: "ignore decorative spinner", model: "gpt-5.6-sol" });
    assert.equal(started.goalActiveObserved, true);
...
test("Codex Goal does not answer unrelated terminal control queries or leak /goal before readiness", async () => {
  ...
  const backend: GoalProcessBackend & { writes: string[]; terminated: boolean } = {
    writes: [], terminated: false,
    async start() { return snapshotFor({ output: "\u001b[5n\u001b[>c\u001b[?25h" }); },
    async write(input) { if (input.chars) this.writes.push(input.chars); return snapshotFor({}); },
    terminate() { this.terminated = true; },
  };
  const manager = scriptedGoalManager(backend, { timeoutMs: 80 });
  try {
    await assert.rejects(manager.start({ workspaceId: "ws_unrelated_terminal_query", workspaceRoot: workspace, goal: "must stay gated", model: "gpt-5.6-sol" }), /produced no output|stable coherent readiness|activation failed/i);
...
test("RED: readiness waits for a resolved model/directory before typing /goal across destructive deltas", async () => {
  const workspace = realpathSync(tmpdir());
  const backend = new ScriptedDeltaBackend([{ output: "\u001b[32mmo" }, { output: "" }, { output: "del: gpt-5.6-sol medium\u001b[0m\n\u001b[34mdir" }, { output: "" }, { output: `ectory: ${workspace}\u001b[0m\nAsk Cod` }, { output: "" }, { output: "ex to do anything\n" }, { output: "" }, { output: "" }]);
  const manager = scriptedGoalManager(backend);
  try { const started = await manager.start({ workspaceId: "ws_delta_fragments", workspaceRoot: workspace, goal: "consume ordered deltas", model: "gpt-5.6-sol" }); assert.equal(started.goalActiveObserved, true); assert.equal(backend.writes.join(""), "/goal consume ordered deltas\r"); } finally { manager.shutdown(); }
});
...
test("RED: startup loading is transient and a fresh coherent repaint can become ready", async () => {
  ...
  const backend = new ScriptedDeltaBackend([{ output: loading }, { output: "" }, { output: `\u001b[2J${ready}` }, { output: "" }, { output: "" }]);
  const manager = scriptedGoalManager(backend);
  try { const started = await manager.start({ workspaceId: "ws_loading_then_ready", workspaceRoot: workspace, goal: "wait through loading", model: "gpt-5.6-sol" }); assert.equal(started.goalActiveObserved, true); assert.equal(backend.writes.join(""), "/goal wait through loading\r"); } finally { manager.shutdown(); }
});

test("RED: later loading revokes readiness but does not become a permanent fatal block", async () => {
  const workspace = realpathSync(tmpdir());
  const ready = `model: gpt-5.6-sol medium\ndirectory: ${workspace}\nAsk Codex to do anything\n`;
  const backend = new ScriptedDeltaBackend([{ output: ready }, { output: "loading\n" }, { output: `\u001b[2J${ready}` }, { output: "" }, { output: "" }]);
  const manager = scriptedGoalManager(backend);
  try { const started = await manager.start({ workspaceId: "ws_ready_loading_ready", workspaceRoot: workspace, goal: "require fresh repaint", model: "gpt-5.6-sol" }); assert.equal(started.goalActiveObserved, true); assert.equal(backend.writes.join(""), "/goal require fresh repaint\r"); } finally { manager.shutdown(); }
});
...
test("RED: startup loading never masks a later trust dialog", async () => {
  ...
  const manager = scriptedGoalManager(backend);
  await assert.rejects(manager.start({ workspaceId: "ws_loading_then_trust", workspaceRoot: workspace, goal: "must not be typed", model: "gpt-5.6-sol" }), /Trust|directory-trust dialog|activation failed/i);
  assert.equal(backend.writes.join(""), ""); assert.equal(backend.terminated, true);
});

test("RED: model and directory without an input-ready prompt never become ready on empty polls", async () => {
  const workspace = realpathSync(tmpdir());
  const backend = new ScriptedDeltaBackend([{ output: `model: gpt-5.6-sol medium\ndirectory: ${workspace}\n` }, { output: "" }, { output: "" }, { output: "" }, { output: "" }], { readyText: `model: gpt-5.6-sol medium\ndirectory: ${workspace}\n` });
  const manager = scriptedGoalManager(backend);
  await assert.rejects(manager.start({ workspaceId: "ws_no_prompt", workspaceRoot: workspace, goal: "must not be typed", model: "gpt-5.6-sol" }), /Codex Goal activation failed|stable coherent readiness|produced no output/);
  assert.equal(backend.writes.join(""), ""); assert.equal(backend.terminated, true); assert.deepEqual(manager.listActiveGoalIds(), []);
});

test("RED: version header cannot substitute for an input-ready prompt", async () => {
  const workspace = realpathSync(tmpdir());
  const backend = new ScriptedDeltaBackend([{ output: `version 1.0\nmodel: gpt-5.6-sol medium\ndirectory: ${workspace}\n` }, { output: "" }, { output: "" }, { output: "" }]);
  const manager = scriptedGoalManager(backend);
  await assert.rejects(manager.start({ workspaceId: "ws_version_without_prompt", workspaceRoot: workspace, goal: "must not be typed", model: "gpt-5.6-sol" }), /Codex Goal activation failed|stable coherent readiness|produced no output/);
  assert.equal(backend.writes.join(""), ""); assert.equal(backend.terminated, true); assert.deepEqual(manager.listActiveGoalIds(), []);
});

test("RED: built-from header cannot substitute for an input-ready prompt", async () => {
  const workspace = realpathSync(tmpdir());
  const backend = new ScriptedDeltaBackend([{ output: `built from source\nmodel: gpt-5.6-sol medium\ndirectory: ${workspace}\n` }, { output: "" }, { output: "" }, { output: "" }]);
  const manager = scriptedGoalManager(backend);
  await assert.rejects(manager.start({ workspaceId: "ws_built_from_without_prompt", workspaceRoot: workspace, goal: "must not be typed", model: "gpt-5.6-sol" }), /Codex Goal activation failed|stable coherent readiness|produced no output/);
  assert.equal(backend.writes.join(""), ""); assert.equal(backend.terminated, true); assert.deepEqual(manager.listActiveGoalIds(), []);
});

for (const [name, hostilePrompt] of [["negated", "not Ask Codex to do anything"], ["suffixed", "Ask Codex to do anything later"], ["diagnostic", "status: Ask Codex to do anything unavailable"], ["embedded", "XAsk Codex to do anythingY"]] as const) {
  test(`RED: ${name} Ask-Codex text cannot substitute for the exact input-ready prompt`, async () => {
    const workspace = realpathSync(tmpdir()); const backend = new ScriptedDeltaBackend([{ output: `model: gpt-5.6-sol medium\ndirectory: ${workspace}\n${hostilePrompt}\n` }, { output: "" }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
    await assert.rejects(manager.start({ workspaceId: `ws_hostile_prompt_${name}`, workspaceRoot: workspace, goal: "must not be typed", model: "gpt-5.6-sol" }), /Codex Goal activation failed|stable coherent readiness|produced no output/);
    assert.equal(backend.writes.join(""), ""); assert.equal(backend.terminated, true); assert.deepEqual(manager.listActiveGoalIds(), []);
  });
}

for (const [name, orderedFrame] of [["before both identity rows", (workspace: string) => `Ask Codex to do anything\nmodel: gpt-5.6-sol medium\ndirectory: ${workspace}\n`], ["between model and directory", (workspace: string) => `model: gpt-5.6-sol medium\nAsk Codex to do anything\ndirectory: ${workspace}\n`], ["between directory and model", (workspace: string) => `directory: ${workspace}\nAsk Codex to do anything\nmodel: gpt-5.6-sol medium\n`]] as const) {
  test(`RED: prompt ${name} cannot establish ordered readiness`, async () => {
    const workspace = realpathSync(tmpdir()); const backend = new ScriptedDeltaBackend([{ output: orderedFrame(workspace) }, { output: "" }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
    await assert.rejects(manager.start({ workspaceId: `ws_prompt_order_${name.replaceAll(" ", "_")}`, workspaceRoot: workspace, goal: "must not be typed", model: "gpt-5.6-sol" }), /Codex Goal activation failed|stable coherent readiness|produced no output/);
    assert.equal(backend.writes.join(""), ""); assert.equal(backend.terminated, true); assert.deepEqual(manager.listActiveGoalIds(), []);
  });
}

test("RED: a later non-prompt identity frame revokes historical prompt readiness", async () => {
  const workspace = realpathSync(tmpdir()); const ready = `model: gpt-5.6-sol medium\ndirectory: ${workspace}\nAsk Codex to do anything\n`; const backend = new ScriptedDeltaBackend([{ output: ready }, { output: `version 1.1\nmodel: gpt-5.6-sol medium\ndirectory: ${workspace}\n` }, { output: "" }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
  await assert.rejects(manager.start({ workspaceId: "ws_prompt_revoked", workspaceRoot: workspace, goal: "must not be typed", model: "gpt-5.6-sol" }), /Codex Goal activation failed|stable coherent readiness|produced no output/);
  assert.equal(backend.writes.join(""), ""); assert.equal(backend.terminated, true); assert.deepEqual(manager.listActiveGoalIds(), []);
});

test("RED: clear-screen delta revokes a ready frame before empty polls", async () => {
  const workspace = realpathSync(tmpdir()); const ready = `model: gpt-5.6-sol medium\ndirectory: ${workspace}\nAsk Codex to do anything\n`; const backend = new ScriptedDeltaBackend([{ output: ready }, { output: "\u001b[2J" }, { output: "" }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
  await assert.rejects(manager.start({ workspaceId: "ws_clear_revokes_ready", workspaceRoot: workspace, goal: "must not be typed", model: "gpt-5.6-sol" }), /Codex Goal activation failed|stable coherent readiness|produced no output/); assert.equal(backend.writes.join(""), ""); assert.equal(backend.terminated, true); assert.deepEqual(manager.listActiveGoalIds(), []);
});

test("RED: clear-screen delta discards partial carry before a later prompt", async () => {
  const workspace = realpathSync(tmpdir()); const ready = `model: gpt-5.6-sol medium\ndirectory: ${workspace}\nAsk Codex to do anything\n`; const backend = new ScriptedDeltaBackend([{ output: ready }, { output: "model:" }, { output: "\u001b[2J" }, { output: ` gpt-5.6-sol medium\ndirectory: ${workspace}\nAsk Codex to do anything\n` }, { output: "" }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
  await assert.rejects(manager.start({ workspaceId: "ws_clear_discards_partial_carry", workspaceRoot: workspace, goal: "must not be typed", model: "gpt-5.6-sol" }), /Codex Goal activation failed|stable coherent readiness|produced no output/); assert.equal(backend.writes.join(""), ""); assert.equal(backend.terminated, true); assert.deepEqual(manager.listActiveGoalIds(), []);
});

test("RED: model and directory after clear require a fresh exact prompt", async () => {
  const workspace = realpathSync(tmpdir()); const ready = `model: gpt-5.6-sol medium\ndirectory: ${workspace}\nAsk Codex to do anything\n`; const backend = new ScriptedDeltaBackend([{ output: ready }, { output: "\u001b[3J" }, { output: `model: gpt-5.6-sol medium\ndirectory: ${workspace}\n` }, { output: "" }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
  await assert.rejects(manager.start({ workspaceId: "ws_clear_requires_fresh_prompt", workspaceRoot: workspace, goal: "must not be typed", model: "gpt-5.6-sol" }), /Codex Goal activation failed|stable coherent readiness|produced no output/); assert.equal(backend.writes.join(""), ""); assert.equal(backend.terminated, true); assert.deepEqual(manager.listActiveGoalIds(), []);
});

test("RED: an ordered frame cleared at the end of one snapshot is not ready", async () => {
  const workspace = realpathSync(tmpdir()); const ready = `model: gpt-5.6-sol medium\ndirectory: ${workspace}\nAsk Codex to do anything\n`; const backend = new ScriptedDeltaBackend([{ output: `${ready}\u001b[2J` }, { output: "" }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
  await assert.rejects(manager.start({ workspaceId: "ws_same_snapshot_clear_at_end", workspaceRoot: workspace, goal: "must not be typed", model: "gpt-5.6-sol" }), /Codex Goal activation failed|stable coherent readiness|produced no output/); assert.equal(backend.writes.join(""), ""); assert.equal(backend.terminated, true); assert.deepEqual(manager.listActiveGoalIds(), []);
});

test("RED: only promptless post-clear identity bytes survive within one snapshot", async () => {
  const workspace = realpathSync(tmpdir()); const ready = `model: gpt-5.6-sol medium\ndirectory: ${workspace}\nAsk Codex to do anything\n`; const backend = new ScriptedDeltaBackend([{ output: `${ready}\u001b[2Jmodel: gpt-5.6-sol medium\ndirectory: ${workspace}\n` }, { output: "" }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
  await assert.rejects(manager.start({ workspaceId: "ws_same_snapshot_promptless_suffix", workspaceRoot: workspace, goal: "must not be typed", model: "gpt-5.6-sol" }), /Codex Goal activation failed|stable coherent readiness|produced no output/); assert.equal(backend.writes.join(""), ""); assert.equal(backend.terminated, true); assert.deepEqual(manager.listActiveGoalIds(), []);
});

test("RED: clear within one snapshot prevents partial-prefix identity synthesis", async () => {
  const workspace = realpathSync(tmpdir()); const backend = new ScriptedDeltaBackend([{ output: `model:\u001b[2J gpt-5.6-sol medium\ndirectory: ${workspace}\nAsk Codex to do anything\n` }, { output: "" }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
  await assert.rejects(manager.start({ workspaceId: "ws_same_snapshot_partial_prefix", workspaceRoot: workspace, goal: "must not be typed", model: "gpt-5.6-sol" }), /Codex Goal activation failed|stable coherent readiness|produced no output/); assert.equal(backend.writes.join(""), ""); assert.equal(backend.terminated, true); assert.deepEqual(manager.listActiveGoalIds(), []);
});

test("RED: a fresh ordered repaint after clear in one snapshot can become ready", async () => {
  const workspace = realpathSync(tmpdir()); const stale = `loading\nmodel: stale-model\ndirectory: /stale\nAsk Codex to do anything\n`; const fresh = `model: gpt-5.6-sol medium\ndirectory: ${workspace}\nAsk Codex to do anything\n`; const backend = new ScriptedDeltaBackend([{ output: `${stale}\u001b[3J${fresh}` }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
  try { const started = await manager.start({ workspaceId: "ws_same_snapshot_fresh_repaint", workspaceRoot: workspace, goal: "accept only the post-clear frame", model: "gpt-5.6-sol" }); assert.equal(started.goalActiveObserved, true); assert.equal(backend.writes.join(""), "/goal accept only the post-clear frame\r"); } finally { manager.shutdown(); }
});

test("RED: a split ESC[2J clear revokes pre-clear identity before a later prompt", async () => {
  const workspace = realpathSync(tmpdir()); const backend = new ScriptedDeltaBackend([{ output: `model: gpt-5.6-sol medium\ndirectory: ${workspace}\n` }, { output: "\u001b[" }, { output: "2J\nAsk Codex to do anything\n" }, { output: "" }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
  await assert.rejects(manager.start({ workspaceId: "ws_split_clear_revokes_identity", workspaceRoot: workspace, goal: "must not be typed", model: "gpt-5.6-sol" }), /Codex Goal activation failed|stable coherent readiness|produced no output/); assert.equal(backend.writes.join(""), ""); assert.equal(backend.terminated, true); assert.deepEqual(manager.listActiveGoalIds(), []);
});

test("RED: a fresh ordered repaint after a split ESC[2J clear can become ready", async () => {
  const workspace = realpathSync(tmpdir()); const fresh = `model: gpt-5.6-sol medium\ndirectory: ${workspace}\nAsk Codex to do anything\n`; const backend = new ScriptedDeltaBackend([{ output: `model: gpt-5.6-sol medium\ndirectory: ${workspace}\n` }, { output: "\u001b[" }, { output: `2J${fresh}` }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
  try { const started = await manager.start({ workspaceId: "ws_split_clear_fresh_repaint", workspaceRoot: workspace, goal: "accept the split-clear repaint", model: "gpt-5.6-sol" }); assert.equal(started.goalActiveObserved, true); assert.equal(backend.writes.join(""), "/goal accept the split-clear repaint\r"); } finally { manager.shutdown(); }
});

test("RED: split clear prevents pre-clear partial identity synthesis", async () => {
  const workspace = realpathSync(tmpdir()); const backend = new ScriptedDeltaBackend([{ output: "model:" }, { output: "\u001b[" }, { output: `2J gpt-5.6-sol medium\ndirectory: ${workspace}\nAsk Codex to do anything\n` }, { output: "" }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
  await assert.rejects(manager.start({ workspaceId: "ws_split_clear_partial_identity", workspaceRoot: workspace, goal: "must not be typed", model: "gpt-5.6-sol" }), /Codex Goal activation failed|stable coherent readiness|produced no output/); assert.equal(backend.writes.join(""), ""); assert.equal(backend.terminated, true); assert.deepEqual(manager.listActiveGoalIds(), []);
});

test("RED: ESC c clear split after ESC revokes pre-clear identity", async () => {
  const workspace = realpathSync(tmpdir()); const backend = new ScriptedDeltaBackend([{ output: `model: gpt-5.6-sol medium\ndirectory: ${workspace}\n` }, { output: "\u001b" }, { output: "c\nAsk Codex to do anything\n" }, { output: "" }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
  await assert.rejects(manager.start({ workspaceId: "ws_split_escape_c_clear", workspaceRoot: workspace, goal: "must not be typed", model: "gpt-5.6-sol" }), /Codex Goal activation failed|stable coherent readiness|produced no output/); assert.equal(backend.writes.join(""), ""); assert.equal(backend.terminated, true); assert.deepEqual(manager.listActiveGoalIds(), []);
});

for (const [name, prefix, suffix] of [["ESC then bracket-2J", "\u001b", "[2J"], ["ESC-bracket-2 then J", "\u001b[2", "J"], ["ESC then bracket-3J", "\u001b", "[3J"], ["ESC-bracket then 3J", "\u001b[", "3J"], ["ESC-bracket-3 then J", "\u001b[3", "J"]] as const) {
  test(`RED: ${name} split clear revokes pre-clear identity`, async () => {
    const workspace = realpathSync(tmpdir()); const backend = new ScriptedDeltaBackend([{ output: `model: gpt-5.6-sol medium\ndirectory: ${workspace}\n` }, { output: prefix }, { output: `${suffix}\nAsk Codex to do anything\n` }, { output: "" }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
    await assert.rejects(manager.start({ workspaceId: `ws_split_clear_${name.replaceAll(/[^a-z0-9]+/gi, "_")}`, workspaceRoot: workspace, goal: "must not be typed", model: "gpt-5.6-sol" }), /Codex Goal activation failed|stable coherent readiness|produced no output/); assert.equal(backend.writes.join(""), ""); assert.equal(backend.terminated, true); assert.deepEqual(manager.listActiveGoalIds(), []);
  });
}

test("RED: a partial later model label revokes historical prompt readiness", async () => {
  const workspace = realpathSync(tmpdir()); const ready = `model: gpt-5.6-sol medium\ndirectory: ${workspace}\nAsk Codex to do anything\n`; const backend = new ScriptedDeltaBackend([{ output: ready }, { output: "model:" }, { output: "" }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
  await assert.rejects(manager.start({ workspaceId: "ws_partial_model_revokes_prompt", workspaceRoot: workspace, goal: "must not be typed", model: "gpt-5.6-sol" }), /Codex Goal activation failed|stable coherent readiness|produced no output/); assert.equal(backend.writes.join(""), ""); assert.equal(backend.terminated, true); assert.deepEqual(manager.listActiveGoalIds(), []);
});

test("RED: a partial later directory label revokes historical prompt readiness", async () => {
  const workspace = realpathSync(tmpdir()); const ready = `model: gpt-5.6-sol medium\ndirectory: ${workspace}\nAsk Codex to do anything\n`; const backend = new ScriptedDeltaBackend([{ output: ready }, { output: "direc" }, { output: "" }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
  await assert.rejects(manager.start({ workspaceId: "ws_partial_directory_revokes_prompt", workspaceRoot: workspace, goal: "must not be typed", model: "gpt-5.6-sol" }), /Codex Goal activation failed|stable coherent readiness|produced no output/); assert.equal(backend.writes.join(""), ""); assert.equal(backend.terminated, true); assert.deepEqual(manager.listActiveGoalIds(), []);
});

test("RED: identical non-empty destructive chunks are consumed twice, not deduplicated", async () => {
  const workspace = realpathSync(tmpdir()); const ready = `model: gpt-5.6-sol medium\ndirectory: ${workspace}\nAsk Codex to do anything\n`; const backend = new ScriptedDeltaBackend([{ output: "" }, { output: ready }, { output: ready }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
  try { const started = await manager.start({ workspaceId: "ws_duplicate_deltas", workspaceRoot: workspace, goal: "consume both chunks" }); assert.ok(backend.nonEmptySnapshotsConsumed >= 2); assert.equal(started.goalActiveObserved, true); } finally { manager.shutdown(); }
});

test("RED: later loading/error/trust output invalidates historical readiness before /goal", async () => {
  const workspace = realpathSync(tmpdir()); const ready = `model: gpt-5.6-sol medium\ndirectory: ${workspace}\nAsk Codex to do anything\n`; const backend = new ScriptedDeltaBackend([{ output: "" }, { output: ready }, { output: "Trust the contents of this directory?\n" }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
  await assert.rejects(manager.start({ workspaceId: "ws_later_trust", workspaceRoot: workspace, goal: "must not be typed" }), /Codex Goal activation failed|did not resolve model and directory|Trust/); assert.equal(backend.writes.join(""), ""); assert.equal(backend.terminated, true); assert.deepEqual(manager.listActiveGoalIds(), []);
});

test("RED: wrong explicit model fails closed without /goal", async () => {
  const workspace = realpathSync(tmpdir()); const backend = new ScriptedDeltaBackend([{ output: "" }, { output: `model: some-other-model medium\ndirectory: ${workspace}\nAsk Codex to do anything\n` }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
  await assert.rejects(manager.start({ workspaceId: "ws_wrong_model", workspaceRoot: workspace, goal: "must not be typed", model: "gpt-5.6-sol" }), /Codex Goal activation failed|did not resolve model and directory/); assert.equal(backend.writes.join(""), ""); assert.deepEqual(manager.listActiveGoalIds(), []);
});

test("RED: wrong workspace directory fails closed without /goal", async () => {
  const workspace = mkdtempSync(join(tmpdir(), "devspace-goal-workspace-")); const otherWorkspace = mkdtempSync(join(tmpdir(), "devspace-goal-other-")); const backend = new ScriptedDeltaBackend([{ output: "" }, { output: `model: gpt-5.6-sol medium\ndirectory: ${otherWorkspace}\nAsk Codex to do anything\n` }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
  try { await assert.rejects(manager.start({ workspaceId: "ws_wrong_dir", workspaceRoot: workspace, goal: "must not be typed" }), /Codex Goal activation failed|did not resolve model and directory/); assert.equal(backend.writes.join(""), ""); assert.deepEqual(manager.listActiveGoalIds(), []); } finally { try { rmSync(workspace, { recursive: true, force: true }); } catch {} try { rmSync(otherWorkspace, { recursive: true, force: true }); } catch {} }
});

test("RED: empty truncated destructive snapshot permanently blocks readiness and leaks no goal", async () => {
  const workspace = realpathSync(tmpdir()); const ready = `model: gpt-5.6-sol medium\ndirectory: ${workspace}\nAsk Codex to do anything\n`; const backend = new ScriptedDeltaBackend([{ output: "" }, { output: ready }, { output: "", outputTruncated: true }, { output: ready }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
  await assert.rejects(manager.start({ workspaceId: "ws_empty_truncated", workspaceRoot: workspace, goal: "must not be typed" }), /Codex Goal activation failed|did not resolve model and directory/); assert.equal(backend.writes.join(""), ""); assert.equal(backend.terminated, true); assert.deepEqual(manager.listActiveGoalIds(), []);
});

test("RED: empty polls do not replay history and one real duplicate is consumed as a real chunk", async () => {
  const workspace = realpathSync(tmpdir()); const ready = `model: gpt-5.6-sol medium\ndirectory: ${workspace}\nAsk Codex to do anything\n`; const backend = new ScriptedDeltaBackend([{ output: "" }, { output: ready }, { output: "" }, { output: "" }, { output: ready }, { output: "" }, { output: "" }]); const manager = scriptedGoalManager(backend);
  try { const started = await manager.start({ workspaceId: "ws_empty_polls", workspaceRoot: workspace, goal: "consume exactly once" }); assert.equal(backend.writes.join(""), "/goal consume exactly once\r"); assert.equal(started.goalActiveObserved, true); } finally { manager.shutdown(); }
});

...
function existingCdpFixture(
  ...
  const calls: Array<{ id: number; method: string; params: any; sessionId?: string }> = [];
  const sockets: FakeSocket[] = [];
  const targets = new Map([
    ["target-a", "https://chatgpt.com/c/worker-a"],
    ["target-b", "https://chatgpt.com/c/worker-b"],
  ]);
  ...
  class FakeSocket {
    ...
    constructor(readonly url: string) {
      sockets.push(this);
      queueMicrotask(() => denyConnection ? this.onclose?.() : this.onopen?.());
    }
    ...
    send(data: string) {
      ...
      calls.push(command);
      if (command.method === disconnectOn) {
        this.close();
        return;
      }
      let result: any = {};
      switch (command.method) {
        ...
        case "Runtime.evaluate": {
          ...
          if (expression === "location.href") {
            ...
            value = domState === "signed-out" ? "SIGNED_OUT" : true;
          }
          result = { result: { value } };
          break;
        }
        default:
          assert.fail(`unexpected CDP command ${command.method}`);
      }
      queueMicrotask(() => this.onmessage?.({
        data: JSON.stringify({
          id: command.id,
          sessionId: wrongSession && command.sessionId
            ? "foreign-session"
            : command.sessionId,
          result,
        }),
  ...
  (globalThis as any).fetch = async () => {
    assert.fail("existing-session must use the metadata browser websocket, never HTTP/default port");
  };
  t.after(() => {
  ...
  const config = loadChatSwarmRuntimeConfig(
    { stateDir: root, chatSwarmMaxWorkers: 3 },
    {
      DEVSPACE_CHAT_SWARM_CDP_MODE: "existing-session",
...
test("existing-session missing or malformed metadata fails closed before connection", async (t) => {
  ...
  await assert.rejects(
    unsafe.createManagedConversation(f.config.projectUrl!, f.deadline()),
...
test("existing-session resolves dynamic browser endpoint anew and closes each bounded connection", async (t) => {
  ...
  writeFileSync(
    join(f.root, "DevToolsActivePort"),
    "45234\r\n/devtools/browser/browser-two\r\n",
  );
  assert.equal((await f.driver.preflight()).ready, true);
  assert.deepEqual(f.sockets.map((socket) => socket.url), [
    "ws://127.0.0.1:43123/devtools/browser/browser-one",
    "ws://127.0.0.1:45234/devtools/browser/browser-two",
  ]);
...
test("existing-session isolates exact targets, reopens lost conversations, and closes only the requested target", async (t) => {
  ...
  assert.deepEqual(
    f.calls.filter((call) => call.method === "Target.attachToTarget").map((call) => call.params.targetId),
    ["target-a"],
  );
  ...
  assert.deepEqual(
    f.calls.filter((call) => call.method === "Target.createTarget").map((call) => call.params.url),
    [url],
  );
...
test("existing-session disconnect before acknowledgement never retries a prompt or switches transport", async (t) => {
  ...
  const result = await f.driver.sendPrompt(
    "https://chatgpt.com/c/worker-a",
    "wake",
    f.deadline(),
  );
  ...
  assert.equal(f.calls.filter((call) => call.method === "Target.createTarget").length, 0);
...
test("existing-session keeps signed-out and app binding blockers separate from control availability", async (t) => {
  ...
  f.dom("signed-out");
  assert.match(
    (await f.driver.recoverConversation("https://chatgpt.com/c/worker-a", f.deadline())).blocker!,
    /CHATGPT_SIGNED_OUT/,
  );
  f.dom("disabled");
  assert.equal(
    (await f.driver.sendPrompt("https://chatgpt.com/c/worker-a", "wake", f.deadline())).blocker,
    "HOST_APP_BINDING_NOT_READY:DISABLED",
  );
});
...
test("existing-session socket close cannot distinguish authorization denial from transport failure", async (t) => {
  ...
  assert.match(
    result.blocker!,
    /BROWSER_CONTROL_UNAVAILABLE:AUTHORIZATION_OR_CONNECTION_CLOSED/,
  );
  assert.equal(f.calls.length, 0);
  assert.equal(f.sockets.length, 1);
  assert.ok(f.sockets.every((socket) => socket.closed));
...
test("existing-session prompt acknowledgement loss remains unknown and is not blindly resent", async (t) => {
  ...
  assert.equal(
    f.calls.filter(
      (call) => call.method === "Runtime.evaluate" && call.params.expression.includes("const prompt="),
    ).length,
    1,
  );
  assert.equal(f.calls.filter((call) => call.method === "Target.createTarget").length, 0);
...
test("existing-session rejects cross-session responses", async (t) => {
  const f = existingCdpFixture(t);
  f.spoofSession();
  const spoofed = await f.driver.recoverConversation(
    "https://chatgpt.com/c/worker-a",
    f.deadline(),
  );
...
test("existing-session checks the exact URL inside the prompt mutation", async (t) => {
  const f = existingCdpFixture(t);
  f.drift();
  const drifted = await f.driver.sendPrompt(
    "https://chatgpt.com/c/worker-a",
    "wake",
    f.deadline(),
  );
...
test("existing-session missing explicit setup is authorization-required, never inferred from missing metadata", async (t) => {
  ...
  for (const browserProfileDir of ["", "relative"]) {
    assert.throws(
      () => loadChatSwarmRuntimeConfig(
        { stateDir: f.root, chatSwarmMaxWorkers: 3 },
        {
          DEVSPACE_CHAT_SWARM_CDP_MODE: "existing-session",
    ...
    await assert.rejects(
      driver.createManagedConversation(f.config.projectUrl!, f.deadline()),
  ...
  assert.equal(
    unavailable.blocker,
    "BROWSER_CONTROL_UNAVAILABLE:DEVTOOLS_ACTIVE_PORT_UNAVAILABLE",
  );
  assert.equal(f.sockets.length, 0);
  assert.equal(f.calls.length, 0);
});

test("existing-session browser readiness surfaces unknown app binding without blocking delivery", async (t) => {
  const f = existingCdpFixture(t);
  f.dom("unknown");
  ...
  assert.deepEqual(await f.driver.sendPrompt(url, "bootstrap", f.deadline()), {
    delivered: true,
    remoteMayContinue: true,
    blocker: "HOST_APP_BINDING_NOT_READY:UNKNOWN",
  });
  ...
  assert.ok(
    f.calls
      ...
      .every((call) => call.params.targetId === "target-a"),
  );
  assert.equal(f.targets.get("target-b"), "https://chatgpt.com/c/worker-b");
});

test("existing-session DISABLED and STALE app binding block send, recovery, and bootstrap", async (t) => {
  const f = existingCdpFixture(t);
  const url = "https://chatgpt.com/c/worker-a";
  ...
  try {
    for (const binding of ["DISABLED", "STALE"] as const) {
      ...
      assert.deepEqual(await f.driver.sendPrompt(url, "wake", f.deadline()), {
        delivered: false,
        remoteMayContinue: false,
        blocker: `HOST_APP_BINDING_NOT_READY:${binding}`,
      });
      assert.deepEqual(await f.driver.recoverConversation(url, f.deadline()), {
        ready: false,
        blocker: `HOST_APP_BINDING_NOT_READY:${binding}`,
...
test("runtime retains DISABLED and STALE binding reasons without bootstrap or reprovision", async (t) => {
  for (const binding of ["DISABLED", "STALE"] as const) {
    const f = fixture();
    try {
      const provision = f.adapter.provision.bind(f.adapter);
  ...
  try {
    t.mock.method(f.adapter, "bootstrap", async () => {
      ...
      return {
        ...
        remoteMayContinue: false,
        blocker: "HOST_APP_BINDING_NOT_READY:DISABLED",
      } as any;
    });
    const status = await f.manager.ensure(f.owner, f.swarm.id, 1);

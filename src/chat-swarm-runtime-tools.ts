...
export function chatSwarmRuntimeToolInputShapes(
  ...
  const desiredWorkers = z
    .number()
    .int()
    .min(1)
    .max(config.chatSwarmMaxWorkers)
  ...
  return {
    ...
    chat_swarm_runtime_bootstrap: { operationId: id.describe("Exact managed provisioning operation created by runtime_ensure.") },
  };
}
...
function requestMeta(extra: { _meta?: unknown }): unknown {
  return extra._meta ?? {};
}
...
export function registerChatSwarmRuntimeTools(
  ...
  const shapes = chatSwarmRuntimeToolInputShapes(config);
  const schemas = Object.fromEntries(
    Object.entries(shapes).map(([name, shape]) => [name, z.object(shape)]),
  ...
  server.registerTool(
    ...
    async (input: any, extra) => {
      try {
        admit("status");
        return success(await manager.status(requestMeta(extra), input.swarmId));
      } catch (error) {
        return failure(error, "runtime_status");
      }
    },
  );
  ...
  server.registerTool(
    ...
    async (input: any, extra) => {
      try {
        ...
        return failure(error, "runtime_ensure");
      }
    },
  );
  ...
  server.registerTool(
    ...
    async (input: any, extra) => {
      try {
        ...
        return failure(error, "runtime_scale");
      }
    },
  );
  ...
  server.registerTool(
    ...
    async (input: any, extra) => {
      try {
        ...
        return failure(error, "runtime_recover");
      }
    },
  );
  ...
  server.registerTool(
    ...
    async (input: any, extra) => {
      try {
        ...
        return failure(error, "runtime_stop");
      }
    },
  );
  ...
  server.registerTool(
    ...
    async (input: any, extra) => {
      try {
        admit("join");
        return success(
          manager.bootstrap(requestMeta(extra), input.operationId),
        );
      } catch (error) {
        return failure(error, "runtime_bootstrap");
      }
    },
  );

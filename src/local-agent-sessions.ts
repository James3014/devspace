...
export class LocalAgentSessionManager {
  ...
  async reconcileAgent(input: ReconcileAgentInput): Promise<ReconcileAgentOutput> {
    ...
    if (hasDurableCodegBinding(record)) {
      ...
      if (!codegConfigured) {
        providerState = "CODEG_UNAVAILABLE";
      } else if (parseCodegTaskHandle(record.providerSessionId) === undefined) {
        // Read-only reconciliation must not call Codeg's folder-history
        // endpoint to rediscover an unbound task. A title lookup is reserved
        // for an explicit mutating cancellation/recovery path.
        providerState = "CODEG_UNBOUND";
      } else {
        try {

...
type CodegMaterializationOperation =
  | { kind: "copy"; file: string; source: string; target: string; mode: number; staged: string }
...
async function materializeCodegTask(
  ...
  const stagingRoot = mkdtempSync(join(tmpdir(), "devspace-codeg-materialize-"));
  const applied: Array<{
    operation: CodegMaterializationOperation;
    backup?: string;
  }> = [];
  ...
  for (const change of changedFiles) {
    if (seenFiles.has(change.file)) {
      throw new Error(`Codeg changed-file manifest contains duplicate path '${change.file}'; refusing materialization.`);
    }
    ...
    if (existsSync(source)) {
      ...
      operations.push({
        ...
        staged: join(stagingRoot, "sources", change.file),
      });
      continue;
    }
  ...
  // Read and stage every source before the first DevSpace workspace mutation.
  // The apply phase below is transactional with explicit rollback, so a
  // failure in one file cannot silently leave an earlier file applied.
  for (const operation of operations) {
    if (operation.kind !== "copy") continue;
    mkdirSync(dirname(operation.staged), { recursive: true });
    copyFileSync(operation.source, operation.staged);
    chmodSync(operation.staged, operation.mode);
  }

  for (const operation of operations) {
    const targetExisted = existsSync(operation.target);
    const backup = targetExisted
      ? join(stagingRoot, "backups", String(applied.length), operation.file)
      : undefined;
    if (backup) {
      mkdirSync(dirname(backup), { recursive: true });
      renameSync(operation.target, backup);
    }
    const entry = { operation, backup };
    applied.push(entry);
    const parent = dirname(operation.target);
    const missingParents: string[] = [];
    let cursor = parent;
    while (!existsSync(cursor) && cursor !== root && cursor.startsWith(`${root}${sep}`)) {
      missingParents.push(cursor);
      cursor = dirname(cursor);
    }
    mkdirSync(parent, { recursive: true });
    createdDirs.push(...missingParents);

    if (operation.kind === "copy") {
      copyFileSync(operation.staged, operation.target);
      chmodSync(operation.target, operation.mode);
    }
  }
  } catch (cause) {
    // Roll back in reverse order. The rollback itself is best-effort but is
    // never hidden: callers receive a materialization failure rather than a
    // false success after a partial apply.
    let rollbackIncomplete = false;
    for (const entry of [...applied].reverse()) {
      try {
        if (existsSync(entry.operation.target)) unlinkSync(entry.operation.target);
        if (entry.backup && existsSync(entry.backup)) {
          mkdirSync(dirname(entry.operation.target), { recursive: true });
          renameSync(entry.backup, entry.operation.target);
        }
      } catch {
        // Preserve the original failure while still attempting every restore.
        rollbackIncomplete = true;
      }
    }
    for (const directory of [...createdDirs].reverse()) {
      try { rmdirSync(directory); } catch (error) {
        // A non-empty directory can be user-owned; leave it intact but make
        // the non-atomic outcome explicit to the caller.
        if ((error as NodeJS.ErrnoException).code !== "ENOTEMPTY") rollbackIncomplete = true;
      }
    }
    const detail = cause instanceof Error ? cause.message : String(cause);
    const rollback = rollbackIncomplete
      ? "rollback was incomplete; manual workspace reconciliation is required"
      : "workspace changes were rolled back";
    throw new Error(
      `Codeg materialization failed atomically for task ${task.id}; ${rollback}: ${detail}`,
      { cause },
    );
  } finally {
...
export async function cancelCodegTask(
  ...
  // A terminal observation after an unacknowledged cancel is not proof that
  // Codeg applied the cancel (the task may have completed independently).
  // Only the provider's explicit canceled state confirms this effect.

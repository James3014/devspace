...
export const NEXUS_CORE_PROTOCOL_CANDIDATE =
  "James3014/nexus-core#29@aabf2d4d00be4a3a357646be97b4d784d486ccb4" as const;
...
export interface CoreMutationAdmission {
  ...
  claim: "UNBOUND_GENERIC_DEVSPACE" | "CORE_BOUND_SESSION";
  pathContainment: "NOT_APPLICABLE" | "STRUCTURED_SINK_ENFORCED" | "NOT_PROVEN";
}
...
export class CoreMutationSessionStore {
  ...
  getById(sessionId: string): CoreMutationSessionRecord | undefined {
  ...
  getActive(workspaceSessionId: string): CoreMutationSessionRecord | undefined {
    const row = this.database.sqlite
  ...
  async open(input: {
    ...
    this.database.sqlite.prepare(`
      insert into core_mutation_sessions (
        id, workspace_session_id, actor_key, binding_id, binding_hash, binding_json,
        source_head, source_tree, status, created_at, updated_at
      ) values (?, ?, ?, ?, ?, ?, ?, ?, 'ACTIVE', ?, ?)
    `).run(
      id,
      input.workspaceSessionId,
  ...
  async admitEffect(input: {
    ...
    if (!active) {
      if (input.pointer?.required || input.pointer?.sessionId || input.pointer?.bindingHash) {
        throw new CoreMutationSessionError("CORE_MUTATION_SESSION_REQUIRED", "No active Core-bound mutation session exists for this workspace.");
      }
      return { bound: false, claim: "UNBOUND_GENERIC_DEVSPACE", pathContainment: "NOT_APPLICABLE" };
    }
    this.assertActor(active, input.actorKey);
    this.assertPointer(active, input.pointer);
    if (input.pointer?.required && (!input.pointer.sessionId || !input.pointer.bindingHash)) {
      throw new CoreMutationSessionError("CORE_MUTATION_POINTER_REQUIRED", "Trusted delegated execution must carry the exact Core session id and binding hash.");
    }
    ...
    `).run(timestamp, timestamp, timestamp, active.id);
    return {
      bound: true,
      sessionId: active.id,
  ...
  async recordCandidate(input: {
    ...
    this.database.sqlite.prepare(`
      insert into core_mutation_candidates (
        ...
        deleted_paths_json, diff_hash, created_at
      ) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
  ...
  async snapshot(input: {
    ...
    this.assertActor(record, input.actorKey);
    return materializeSnapshot(input.workspaceRoot, record);
  }
}
...
async function treeEntry(
  ...
  const output = await runGit(workspaceRoot, ["ls-tree", "-z", tree, "--", path]);
...
async function materializeSnapshot(
  ...
  let targetTree = currentHeadTree;
  let materializedGitObjects = false;

  if (dirty) {
    const tempDir = await mkdtemp(join(tmpdir(), "devspace-core-mutation-index-"));
    const indexPath = join(tempDir, "index");
    const env = { GIT_INDEX_FILE: indexPath };
    try {
      await runGit(workspaceRoot, ["read-tree", "HEAD"], { env });
      await runGit(workspaceRoot, ["add", "-A", "--", "."], { env });
      targetTree = (await runGit(workspaceRoot, ["write-tree"], { env })).trim();
  ...
  for (const change of changes) {
    const before = await treeEntry(workspaceRoot, sourceTreeOid, change.path);
    const after = await treeEntry(workspaceRoot, targetTree, change.path);
  ...
  const diffHash = coreCanonicalHash([
    ...
    changeManifest.source_tree,
    changeManifest.target_tree,
    entries.map((entry) => [
      entry.path,
      entry.change_type,
      entry.before_oid,

...
function rowLease(row: LeaseRow): ResourceLease {
  ...
  if (!Array.isArray(scope) || !grant.repository || !grant.goal || !grant.coordinatorThread) throw new ControlPlaneOwnershipError("MALFORMED", "lease evidence is malformed");
  if (!Number.isInteger(row.version) || row.version < 1 || (row.operation_state !== null && row.operation_state !== "active" && row.operation_state !== "finished")) throw new ControlPlaneOwnershipError("MALFORMED", "lease state is malformed");
...
export class ControlPlaneOwnershipStore {
  ...
  acquire(consumerContext: unknown, input: ResourceLeaseInput): ResourceLease {
    ...
    const tx = this.sqlite.transaction(() => {
      ...
      if (existing) {
        ...
        if (current.ownerThread === owner.ownerThread && current.baseRevision === input.baseRevision && JSON.stringify(current.scope) === JSON.stringify(scope) && current.terminalState === undefined) return current;
  ...
  finishOperation(consumerContext: unknown, leaseId: string, expectedVersion: number, handle: string): ResourceLease { bounded(handle, "operation handle"); const owner = ownerFor(this.options, consumerContext); const now = new Date(this.now()).toISOString(); const result = this.sqlite.prepare("update control_plane_resource_leases set active_operation_handle=null, operation_state='finished', version=version+1, updated_at=? where lease_id=? and owner_thread=? and version=? and active_operation_handle=? and terminal_state is null").run(now, leaseId, owner.ownerThread, expectedVersion, handle); if (result.changes !== 1) throw new ControlPlaneOwnershipError("CAS_CONFLICT", "operation completion no longer matches"); return rowLease(this.sqlite.prepare("select * from control_plane_resource_leases where lease_id=?").get(leaseId) as LeaseRow); }

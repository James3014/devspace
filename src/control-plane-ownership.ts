...
export class ControlPlaneOwnershipStore {
  ...
  beginOperation(consumerContext: unknown, leaseId: string, expectedVersion: number, handle: string): ResourceLease { bounded(handle, "operation handle"); const owner = ownerFor(this.options, consumerContext); const now = new Date(this.now()).toISOString(); const tx = this.sqlite.transaction(() => { const row = this.sqlite.prepare("select * from control_plane_resource_leases where lease_id=?").get(leaseId) as LeaseRow | undefined; if (!row) throw new ControlPlaneOwnershipError("OWNERSHIP_CONFLICT", "lease not found"); const lease = rowLease(row); this.assertPhysicalBinding(lease); verifyGrant(this.options, lease.grant, owner); this.assertCurrentGrant(lease.grant, lease.grantVersion); if (lease.ownerThread !== owner.ownerThread || lease.version !== expectedVersion || lease.terminalState || lease.operationHandle) throw new ControlPlaneOwnershipError("CAS_CONFLICT", "operation pin no longer matches"); if (Date.parse(lease.expiresAt) <= this.now()) throw new ControlPlaneOwnershipError("EXPIRED", "lease is expired and requires reconciliation"); this.assertNoOverlap(lease, lease.leaseId); const result = this.sqlite.prepare("update control_plane_resource_leases set active_operation_handle=?, operation_state='active', version=version+1, updated_at=? where lease_id=? and owner_thread=? and version=? and terminal_state is null and active_operation_handle is null").run(handle, now, leaseId, owner.ownerThread, expectedVersion); if (result.changes !== 1) throw new ControlPlaneOwnershipError("CAS_CONFLICT", "operation pin raced"); return rowLease(this.sqlite.prepare("select * from control_plane_resource_leases where lease_id=?").get(leaseId) as LeaseRow); }); return tx.immediate(); }
  ...
  handoff(consumerContext: unknown, leaseId: string, expectedVersion: number, recipientContext: unknown, input: HandoffInput): HandoffReceipt {
    ...
    return this.sqlite.transaction(() => {
      ...
      if (lease.operationHandle !== undefined || lease.operationState === "active") throw new ControlPlaneOwnershipError("OWNERSHIP_CONFLICT", "reconcile the pinned operation before handoff");
      ...
      const result = this.sqlite.prepare("update control_plane_resource_leases set owner_thread=?,grant_json=?,grant_version=?,expires_at=?,version=version+1,updated_at=? where lease_id=? and owner_thread=? and version=? and terminal_state is null and active_operation_handle is null and (operation_state is null or operation_state <> 'active')")

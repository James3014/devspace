...
async function main() {
  const swarmId = "swarm_798e609cfe7d4a78a7cefb5f5fb05ecd";
  ...
  const clientMetadata = {
    client_name: "devspace-probe-status",
  ...
  const client = new Client({ name: "devspace-probe-status", version: "1.0.0" });
  await client.connect(transport);

  const statusRes = await client.callTool({
    name: "chat_swarm_runtime_status",
    arguments: { swarmId },
    _meta: { "openai/session": "v1/3b9C3Q11WLxf0wujKVKoSdsoxgJvAfbWXRG0cavUps9o1sXdaeZdGlwLmcUEXKdRnfHlKC1drP6l" },
  });
  console.log("Runtime Status:", JSON.stringify(statusRes, null, 2));

...
async function main() {
  ...
  const clientMetadata = {
    client_name: "devspace-probe-verify-slot1",
  ...
  const client = new Client({ name: "devspace-probe-verify-slot1", version: "1.0.0" });
  await client.connect(transport);
  console.log("Connected to MCP server!");

  // Step 1: Create fresh swarm
  const createRes: any = await client.callTool({
    name: "chat_swarm_create",
    arguments: { workerLimit: 5 },
    _meta: { "openai/session": "v1/3b9C3Q11WLxf0wujKVKoSdsoxgJvAfbWXRG0cavUps9o1sXdaeZdGlwLmcUEXKdRnfHlKC1drP6l" },
  });
  const swarmId = createRes.structuredContent?.swarm?.id || JSON.parse(createRes.content[0].text)?.swarm?.id;
  console.log(`Created new swarm: ${swarmId}`);
  if (!swarmId) {
    console.error("Failed to extract swarmId from:", createRes);
    process.exit(1);
  }

  // Step 2: Call chat_swarm_runtime_ensure(desiredWorkers=1)
  console.log("Calling chat_swarm_runtime_ensure(desiredWorkers=1)...");
  console.log("Expected: 1 Chrome tab opens, prompts @dev2, waits in-tab for ChatGPT to call tool and reply (~15s).");
  const ensureRes = await client.callTool({
    name: "chat_swarm_runtime_ensure",
    arguments: { swarmId, desiredWorkers: 1 },
    _meta: { "openai/session": "v1/3b9C3Q11WLxf0wujKVKoSdsoxgJvAfbWXRG0cavUps9o1sXdaeZdGlwLmcUEXKdRnfHlKC1drP6l" },
  });
  console.log("Ensure Result:");
  console.log(JSON.stringify(ensureRes, null, 2));

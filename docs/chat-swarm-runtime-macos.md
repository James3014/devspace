...
One-time setup requires Chrome remote debugging to be explicitly enabled and the user to authorize the connection in Chrome. Consent remains under Chrome/user control; DevSpace does not automate either step.
...
- `BROWSER_AUTHORIZATION_REQUIRED:*` is reserved for known setup evidence such as missing explicit existing-session selection inputs. A missing endpoint artifact or closed socket alone is not proof that Chrome authorization was denied.
...
- `HOST_APP_BINDING_NOT_READY:*`, `PEER_IDENTITY_UNRESOLVED:*`, `CHATGPT_CONVERSATION_IDENTITY_DRIFT`, authenticated peer checks, and durable `RECONCILE_REQUIRED` retain separate meanings. Control connectivity alone proves none of them.
